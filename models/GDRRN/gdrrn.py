...
class GDRRN(nn.Module):
		def __init__(self, channels, scale_factor):
				...
				self.input = nn.Conv2d(in_channels=channels, out_channels=num_chnl, kernel_size=3, stride=1, padding=1, bias=False, groups=1)
				...
				self.output = nn.Conv2d(in_channels=num_chnl, out_channels=channels, kernel_size=3, stride=1, padding=1, bias=False, groups=1)
				self.relu = nn.ReLU(inplace=True)
