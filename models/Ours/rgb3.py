...
class Layer(nn.Module):
	...
	def forward(self, x, hsi_mode=True):
		x = self.layer1(x) + x
		x = self.layer3(self.layer2(x)) + x
		return x
...
class Block(nn.Module):
	def __init__(self, cfgs):
		...
		self.hsi_sa = BandSALayer3D(channels, n_feats, cfgs, act=act)
		self.rgb_sa = BandSALayer3D(3, n_feats, cfgs, act=act)
