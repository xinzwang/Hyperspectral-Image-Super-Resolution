...
class Block(nn.Module):
	...
	def forward_hsi(self, x):
		...
		out = self.band_sa(out, rgb_mode=False)
		return out + x
	...
	def forward_rgb(self, x):
		...
		out = self.band_sa(out, rgb_mode=True)
		return out + x


class OursRGB5(nn.Module):
	def __init__(self, channels, scale_factor):
		...
		self.conv_after_body = nn.Conv3d(n_feats, n_feats, kernel_size=(1,1,1), padding=(0,0,0))
