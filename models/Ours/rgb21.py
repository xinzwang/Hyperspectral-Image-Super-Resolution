...
class Layer(nn.Module):
	def __init__(self, cfgs):
		...
		self.layer3 = CALayer2DFeatsDimWithChansPosEmbed(channels=channels, n_feats=n_feats, reduction=reduction)
	
	def forward(self, x, rgb_mode=False):
		x = self.layer1(x) + x 	# 纯RCAN Block可能会好一点？
		x = self.layer3(self.layer2(x), rgb_mode) + x
		return x
...
class Block(nn.Module):
	...
	def forward_rgb(self, x):
		out = x
		for l in self.layers:
			out = l(out, rgb_mode=True)
		out = self.rgb_conv(out) + x
		return out


class OursRGB21(nn.Module):
