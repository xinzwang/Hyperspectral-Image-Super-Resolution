...
def cal_metrics2(pred, hr, max_v=1.0):
	psnr, ssim, sam, ergas = [], [], [], []
	for p, h in zip(pred, hr):
		metric = iv.spectra_metric(p, h, max_v=max_v)
		
		psnr.append(metric.PSNR())
		ssim.append(metric.SSIM())
		sam.append(metric.SAM())
		ergas.append(metric.ERGAS())
...
def cal_metrics(pred, hr, max_v=1.0, ratio=4):
	psnr, ssim, sam, ergas = [], [], [], []
	for p, h in zip(pred, hr):
		res = quality_assessment(x_true=h, x_pred=p, data_range=max_v, ratio=ratio)
		psnr.append(res['MPSNR'])
		ssim.append(res['MSSIM'])
		sam.append(res['SAM'])
		ergas.append(res['ERGAS'])
	return psnr, ssim, sam, ergas
